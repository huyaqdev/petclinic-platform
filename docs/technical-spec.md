...
> **Version note:** Originally specified as `1.29`. By the time PETPLAT-15/-16 were deployed (2026-07), `1.29` had aged out of EKS support entirely (`aws eks describe-cluster-versions` showed `1.30`-`1.32` on paid extended support and `1.33`-`1.36` on standard support) — bumped to `1.33`, the oldest version still on free standard support. Re-check `aws eks describe-cluster-versions` before reusing this spec much later. This also forced the node AMI type from `AL2_ARM_64` to `AL2023_ARM_64_STANDARD` — AL2-family AMIs are rejected outright by EKS for node groups on K8s 1.33+ (`InvalidParameterException: AMI Type AL2_ARM_64 is only supported for kubernetes versions 1.32 or earlier`).
